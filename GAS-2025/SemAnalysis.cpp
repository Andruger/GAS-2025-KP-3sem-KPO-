	...
	bool Semantic::semanticsCheck(Lexer::LEX& tables, Log::LOG& log)
	{
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
			...
			case LEX_DIRSLASH:
			{
				...
				if (tables.lextable.table[i + 1].lexema == LEX_ID)
				{
					for (k; k > 0; k--)
					{
						if (tables.lextable.table[k].lexema == LEX_ID)
						{
							...
							{
								if (tables.lextable.table[k + 2].lexema == LEX_LITERAL && tables.idtable.table[tables.lextable.table[k + 2].idxTI].value.vnum == 0)
								{
									sem_ok = false;
				...
				{
					if (tables.idtable.table[tables.lextable.table[i + 1].idxTI].value.vnum == 0)
					{
						sem_ok = false;
						Log::writeError(log.stream, Error::GetError(318, tables.lextable.table[k].sn, 0));
					}
			...
			case LEX_MORE:	case LEX_LESS: case LEX_EQUALS:   case LEX_NOTEQUALS:	case LEX_MOREEQUALS:	case LEX_LESSEQUALS:
			{
				...
				{
					if (tables.idtable.table[tables.lextable.table[i - 1].idxTI].iddatatype != IT::IDDATATYPE::SHORT)
						flag = false;
				}
