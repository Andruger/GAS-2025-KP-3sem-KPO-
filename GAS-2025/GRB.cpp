...
namespace GRB
{
	...
	Greibach greibach(NS('S'), TS('$'), 16,
		...
		Rule(NS('Z'), GRB_ERROR_SERIES + 9, 8,					// Логические операции	
			Rule::Chain(3, TS('i'), NS('L'), TS('i')),		// переменная оператор переменная
			Rule::Chain(3, TS('i'), NS('L'), TS('l')),		// переменная оператор литерал
			Rule::Chain(3, TS('l'), NS('L'), TS('i')),		// литерал оператор переменная
			Rule::Chain(1, TS('T')),							// true
			Rule::Chain(1, TS('F')),							// false
			Rule::Chain(1, TS('i')),							// идентификатор (переменная)
			Rule::Chain(1, TS('l')),							// литерал (число, например 1 или 0)
			Rule::Chain(1, TS('h'))							// hex литерал
		),
		...
		// Добавлена поддержка нового синтаксиса цикла: do [...] # is: условие
		// -------------------------------------------------------------------------------------
		Rule(NS('K'), GRB_ERROR_SERIES + 14, 18,
			...
			Rule::Chain(9, TS('c'), TS('['), NS('X'), TS(']'), TS('#'), TS('?'), NS('Z'), TS(';'), NS('K')),	// do [тело] # is: условие #;
			...
			Rule::Chain(7, TS('c'), TS('['), NS('X'), TS(']'), TS('#'), TS('?'), NS('Z'), TS('#'))	// Терминал: do [тело] # is: условие #
		),
